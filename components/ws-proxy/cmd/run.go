...
import (
	...
	"github.com/gitpod-io/gitpod/ws-proxy/pkg/utils"
)
...
var runCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(cfg.WSManagerProxy.ListenAddress) > 0 {
			go func() {
...
func startWSManagerTCPProxy(ctx context.Context, listenAddr string, wsManagerAddr string, refillInterval time.Duration, bucketSize int) error {
	...
	p.ListenFunc = utils.NewRateLimitedListen(ctx, refillInterval, bucketSize)
